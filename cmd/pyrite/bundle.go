...
func cmdBundleExport(args []string) error {
	...
	fmt.Printf("  %-16s %s, %d bars\n", "carries", countOf(len(man.Series), "symbol"), totalBars(man))
...
func cmdBundleRun(args []string) error {
	...
	fmt.Printf("  %-16s %s, %d bars, from %s\n", "data", countOf(len(b.Manifest.Series), "symbol"),
...
func cmdBundleShow(args []string) error {
	...
	if m.Index != "" {
		...
		fmt.Printf("  %-16s %s\n", "universe", countOf(len(b.Spec.Universe), "symbol"))
	}
	...
	for i, s := range m.Series {
		if i >= maxRows {
			fmt.Printf("  ... and %s more; --json lists them all\n",
				countOf(len(m.Series)-maxRows, "symbol"))
			break
		}
...
// countOf renders a count with its noun, because "1 symbols" reads as a bug.
// Named for what it returns rather than countOf(), which is a different
// helper in this package returning the noun alone.
